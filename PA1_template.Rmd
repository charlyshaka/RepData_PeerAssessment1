...
```{r}
data <- read.csv("C:/Users/Administrador/Documents/GitHub/RepData_PeerAssessment1/activity.csv")
```
...
b=aggregate(data$steps,by=list(data$date),FUN=sum)
...
```{r}
c=aggregate(data$steps,by=list(data$interval),FUN=mean,na.rm=TRUE)
...
```{r}
m=aggregate(data$steps,by=list(data$date),FUN=sum)
